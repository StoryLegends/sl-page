...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || mobileMenuOpen ? 'py-4 bg-black/80 backdrop-blur-md' : 'py-6 bg-transparent'
      }`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        {/* Mobile Menu Drawer */}
        <div className={`fixed top-0 right-0 h-full w-64 bg-black border-l border-white/10 shadow-2xl z-[100] transition-transform duration-300 md:hidden flex flex-col ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}>
          ...
          <div className="flex flex-col items-center gap-8 mt-10">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.href}
                className={`text-xl font-bold transition-colors ${location.pathname === link.href ? 'text-white' : 'text-gray-400 hover:text-white'
                  }`}
              >
                {link.name}
              </Link>
